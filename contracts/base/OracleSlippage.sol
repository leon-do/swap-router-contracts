...
import '@etcswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@etcswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@etcswap/v3-periphery/contracts/libraries/Path.sol';
...
import '@etcswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IETCswapV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IETCswapV3Pool pool) {
        pool = IETCswapV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IETCswapV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IETCswapV3Pool(pool).slot0();
            }
